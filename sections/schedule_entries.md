...
Subscribers work the same way: omit both `subscriptions` and `notify` and a drafted entry keeps its current subscribers; send either one to recompute the list. The creator is always on the list.
